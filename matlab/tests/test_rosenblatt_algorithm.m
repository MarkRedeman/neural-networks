%% test_rosenblatt_algorithm:
function [tests] = test_rosenblatt_algorithm(t)
...
function [] = test_it_finds_a_linear_separation_of_a_1_dimensional_data_set(t)
    ...
    assert(length(w) == 1);
    assert_energy_aove_treshold(w, labels, samples);
end
...
function [] = test_uses_max_steps_for_a_set_that_is_not_separable(t)
    ...
    assert_energy_not_aove_treshold(w, labels, samples);
end

%% assert_energy_aove_treshold:
function [] = assert_energy_aove_treshold(weight, labels, samples)
...
function [] = assert_energy_not_aove_treshold(weight, labels, samples)
    sample_size = size(samples, 1);
